...
const require_premium_decorator_1 = require("../decorators/require-premium.decorator");
let PremiumGuard = class PremiumGuard {
    ...
    async canActivate(context) {
        const isRequired = this.reflector.getAllAndOverride(require_premium_decorator_1.IS_PREMIUM_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        if (isRequired === false) {
        ...
        if (!active) {
            throw new common_1.ForbiddenException('This feature requires an active Premium subscription.');
        }
        return true;
    }
};
