...
/**
 * Telegram chat interfeysi ko'rinishidagi PDF generatsiya qilish:
 * - Jo'natilgan xabarlar (owner) O'NGDA
 * - Qabul qilingan xabarlar (partner) CHAPDA
 * - Telegram ranglari (fon, bubblar, vaqt, o'chirilgan va tahrirlangan holatlar)
 */
export async function generateChatPdf(options: GenerateChatPdfOptions): Promise<string> {
  return new Promise((resolve, reject) => {
    try {
      ...
      const margin = 36;
      const printableWidth = pageWidth - margin * 2; // 523.28
      ...
      // Har bir sahifaga Telegram fonini chizish funksiyasi
      const drawWallpaper = () => {
        doc.rect(0, 0, pageWidth, pageHeight).fill('#eef3f8');
      };

      // 1-sahifa fonini chizish
      drawWallpaper();
      ...
      doc.fillColor('#7dd3fc').fontSize(9.5).text(
        `Chat: ${sanitizeText(options.chatTitle)} | Telegram ID: ${options.chatId}`,
        36,
        36
      );
      doc.fillColor('#94a3b8').fontSize(8).text(
        `Davr: ${fromStr} — ${toStr} | Jami xabarlar: ${totalCount} ta | O'chirilgan: ${deletedCount} ta | Tahrirlangan: ${editedCount} ta`,
        36,
        52
      );
      ...
      if (totalCount === 0) {
        ...
        for (let i = 0; i < options.messages.length; i++) {
          ...
          if (dateKey !== lastDateKey) {
            ...
            // Agar sahifa oxiriga yaqin bo'lsa, yangi sahifa ochamiz
            if (doc.y > 750) {
              doc.addPage();
              drawWallpaper();
              doc.y = 36;
          ...
          if (isOutgoing) {
            ...
            senderLabel = `${msg.sender_first_name}${msg.sender_last_name ? ' ' + msg.sender_last_name : ''}${
          ...
          const bubbleY = doc.y;
          // O'ngda (outgoing) yoki Chapda (incoming)
          ...
          // Kimdan (Sender) va Holat (Deleted/Normal)
          const senderColor = isOutgoing ? '#15803d' : '#0284c7';
          doc.fillColor(senderColor).fontSize(8.5).text(
            senderLabel,
            bubbleX + padding,
            bubbleY + 6,
            { width: bubbleWidth - padding * 2 - (isDeleted ? 80 : 0) }
          );

          if (isDeleted) {
            doc.fillColor('#dc2626').fontSize(7.5).text(
              '[O\'CHIRILGAN]',
              bubbleX + bubbleWidth - padding - 80,
              bubbleY + 6,
          ...
          if (msg.media_type) {
            ...
            doc.fillColor('#334155').fontSize(8).text(
              `📎 ${sanitizeText(msg.media_type)}`,
              bubbleX + padding + 6,
              currentContentY + 4,
              { width: bubbleWidth - padding * 2 - 12 }
            );
          ...
          if (textContent) {
            ...
            doc.fillColor(textColor).fontSize(9.5).text(
              textContent,
              bubbleX + padding,
              currentContentY,
              { width: contentWidth }
          ...
          if (isEdited && msg.edit_history && msg.edit_history.length > 0) {
            ...
            doc.fillColor('#b45309').fontSize(7.5).text(
              'Asl matn (tahrirlanishdan oldin):',
              bubbleX + padding + 5,
              currentContentY + 3
            );
            ...
            for (const h of msg.edit_history) {
              ...
              doc.fillColor('#78350f').fontSize(7.5).text(
                `• "${sanitizeText(h.text)}" ${hTime ? `(${hTime})` : ''}`,
                bubbleX + padding + 8,
                histItemY,
                { width: bubbleWidth - padding * 2 - 16 }
              );
              histItemY += 13;
            }
      ...
      for (let p = 0; p < pages.count; p++) {
        ...
        doc.fillColor('#64748b').fontSize(8).text(
          'TrackMyChatBot — Xavfsiz Telegram Chat Audit Hisoboti',
          36,
          812,
          { align: 'left' }
        );
